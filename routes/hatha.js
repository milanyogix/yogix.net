...
const hathaRoutes = express.Router();
const yogaData = require('../app/model/hatha');
var moment = require('moment');

hathaRoutes.post('/hathayogadata', async(req, res) => {
...
hathaRoutes.get('/hathayogadata', async(req, res) => {
...
hathaRoutes.get('/hathasessiondata', async(req, res) => {
    var date = req.query.date;
    var email = req.query.email.replace(/["]+/g, '');
    var SetNo = parseInt(req.query.SetNo);
    var SessionNo = parseInt(req.query.SessionNo);
    yogaData.find({ $and: [{ insertDateTime: date }, { email: email }, { SetNo: SetNo }, { SessionNo: SessionNo }] }, (err, data) => {
        if (err) throw err;
        if (!data) {
            res.send("Data not found");
        }
...
hathaRoutes.get('/hathasetdata', async(req, res) => {
    var date = req.query.date;
    var email = req.query.email.replace(/["]+/g, '');
    var SessionNo = parseInt(req.query.SessionNo);
    // var SetNo = parseInt(req.query.SetNo);
...
hathaRoutes.get('/hathaposedata', async(req, res) => {
